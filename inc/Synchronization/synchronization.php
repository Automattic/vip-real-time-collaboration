...
class Synchronization {
	...
	public static function init(): void {
		add_filter( 'heartbeat_received', [ __CLASS__, 'gutenberg_sync_heartbeat' ], 10, 2 );
		add_filter( 'wp_insert_post_data', [ __CLASS__, 'gutenberg_filter_post_content_ydoc' ], 10, 1 );
	}
	 ...
	 * lead to data loss.
	 *
	 * @param mixed $data The post data to be filtered.
	 * @return mixed The filtered post data.
	 */
	public static function gutenberg_filter_post_content_ydoc( mixed $data ): mixed {
	 ...
	 *
	 * @param string $content The HTML content to extract y:gutenberg data from.
	 * @return array|null The extracted y:gutenberg data or null if not found.
	 */
	private static function gutenberg_get_yinfo( string $content ): ?array {
	 ...
	 * @param array $data The heartbeat data array.
	 * @return array The modified response array.
	 */
	public static function gutenberg_sync_heartbeat( array $response, array $data ): array {
			...
			if ( strcmp( $posttype, 'postType/Posts' ) === 0 ) {
				...
				foreach ( $requested_docs as $postid => $expected_client_id ) {
					...
					if ( $post ) {
						...
						$yinfo = self::gutenberg_get_yinfo( $postcontent );
